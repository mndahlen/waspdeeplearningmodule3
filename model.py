...
class WaveGrad_light(nn.Module):
  def __init__(self, params):
    super().__init__()
    self.params = params
    self.downsample = nn.ModuleList([
        Conv1d(1, 32, 5, padding=2), # A
        DBlock(32, 64, 2), # B
        DBlock(64, 64, 2), # C
        DBlock(64, 128, 3), # D
        DBlock(128, 256, 5), # E
    ...
    self.upsample = nn.ModuleList([
        UBlock(768, 256, 5, [1, 2, 1, 2]), # E
        UBlock(256, 256, 5, [1, 2, 1, 2]), # D
        UBlock(256, 128, 3, [1, 2, 4, 8]), # C
        UBlock(128, 64, 2, [1, 2, 4, 8]), # B
        UBlock(64, 64, 2, [1, 2, 4, 8]), # A
    ])
    self.first_conv = Conv1d(128, 768, 3, padding=1)
